...
3. Register the trained model into the MLflow Model Registry
...
- Inference happens through an API
...
- Logs:
  ...
  - Artifacts:
    ...
    - scaler (scaler.pkl)
- Registers model into the MLflow Model Registry as `ClassificationModel`

### Dockerized Services
- `mlflow_server`: MLflow UI + backend store
- `trainer`: trains model and logs experiments (runs once and exits successfully)
- `model_api`: serves predictions using the latest registered model
...
Trainer container will run once, log experiments, then exit with code 0
...
Multiple runs with different hyperparameters (C values)

Logged metrics and artifacts

Registered model: ClassificationModel

Build and Run Dockerized API

The API starts automatically when you run:
...
mlflow_server running on port 5000

model_api running on port 8000
...
Example Request (PowerShell / Linux / Git Bash)
curl -X POST http://localhost:8000/predict \
  -H "Content-Type: application/json" \
  -d '{"features":[[17.99,10.38,122.8,1001,0.1184,0.2776,0.3001,0.1471,0.2419,0.07871,1.095,0.9053,8.589,153.4,0.006399,0.04904,0.05373,0.01587,0.03003,0.006193,25.38,17.33,184.6,2019,0.1622,0.6656,0.7119,0.2654,0.4601,0.1189]]}'
...
Note: The API returns the predicted class label (0/1) based on the trained classification model.
...
If unit tests are added, you can run them inside the API container:
...
MLflow is used for experiment tracking, reproducibility, and model versioning.
...
Flask is used to provide lightweight inference endpoints suitable for local testing and deployment.